...
from aiogram_dialog.widgets.kbd import Button, Row, Column, Cancel, Url, Calendar
...
async def on_date_input(callback: CallbackQuery, widget, manager: DialogManager, selected_date: date):
    manager.dialog_data["date"] = selected_date.isoformat()
    await manager.switch_to(CreateEvent.confirm)

async def on_song_picked(callback: CallbackQuery, button: Button, manager: DialogManager, song_id: str):
...
router.include_router(Dialog(
    ...
    Window(
        ...
        Button(Const("Не будет"), id="tracklist_off", on_click=on_tracklist_disable),
        Cancel(Const("Отмена")),
    ...
    Window(
        ...
        Button(Const("Подтвердить"), id="confirm_tracklist", on_click=on_confirm_tracklist),
        Cancel(Const("Отмена")),
    ...
    Window(
        ...
        Format("\n{verbose_tracklist}"),
        Format("\nДата: {date}\n"),
        Button(Const("Подтвердить"), id="confirm_event", on_click=on_confirm_event),
        Cancel(Const("Отмена")),
