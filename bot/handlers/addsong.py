...
async def add_song(
    ...
    await callback.bot.send_message(chat_id=settings.CHAT_ID, text=(
...
router.include_router(
    Dialog(
        ...
        Window(
            ...
            MessageInput(content_types=ContentType.TEXT, func=on_role_input, id="input_seeking_role"),
            Button(Const("Удалить последнюю роль"), id="pop_last", on_click=remove_last_added_role, when="can_remove_last"),
            Button(Const("Дальше"), id="to_verify", on_click=lambda c, b, m: m.switch_to(AddSong.verify)),
            Cancel(Const("Отмена")),
